...
RUN apt-get update \
    ...
    && useradd \
        ...
        --no-create-home \
        inventory \
    && install --directory --owner=10001 --group=10001 /opt/inventory

WORKDIR /opt/inventory

COPY --from=build --chown=10001:10001 /workspace/target/inventory.jar /opt/inventory/inventory.jar

...
readonly PROJECT_NAME="rentflow-inventory-smoke"
...
readonly CREATE_REQUEST='{"serialNumber":"SMOKE-001","type":"Industrial drill","name":"Smoke drill","status":"AVAILABLE"}'
...
assert_bootstrap_state() {
    ...
    role_state="$(compose exec --no-TTY rentflow-postgres \
        ...
        ")"
    [[ "$role_state" == "${inventory_user}:true:false:false:false" ]] \
        || fail "Inventory role/schema bootstrap state is invalid"
...
create_item() {
    ...
    [[ "$response" == "$EXPECTED_ITEM" ]] || fail "Create response did not match the inventory contract"
}
...
assert_item_readable() {
    ...
    response="$(curl \
        ...
        --show-error \
        --max-time 5 \
        "${BASE_URL}/api/v1/inventory/${SERIAL_NUMBER}")"
    [[ "$response" == "$EXPECTED_ITEM" ]] || fail "Persisted inventory item could not be retrieved"
}
...
info "Building the Inventory runtime image"
compose build inventory
...
compose up --detach inventory
wait_for_service_health inventory 120
assert_runtime_image

info "Creating and retrieving an inventory item"
create_item
assert_item_readable

info "Restarting Inventory and checking database-backed persistence"
